...
then re-fetch. `jsearch_queries` are likewise generated from the submitted
titles + skills + locations, so domain tuning belongs in the *skills* answer,
not in sources.json.

**Pending: a revised salary floor and an added location are currently
overrides.** The candidate relayed both verbally; they were written straight
into profile.json to test the effect. They must go through the form to survive a
re-fetch — see the source-of-truth rule above.
