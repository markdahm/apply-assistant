...
# The guided tour behind the Desk's Help button. Self-contained (the annotated
# screenshot is an inlined data URI), so it is just a static page — but that
# screenshot shows the candidate's real queue, which is why the built file is
# gitignored. Rebuild it with review-app/guide-template.html when the UI moves.
if [ -f ../review-app/guide.html ]; then
  cp ../review-app/guide.html guide.html
else
  echo "  !! no review-app/guide.html — the Help button will 404"
fi
